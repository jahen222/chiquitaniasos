$(document).ready(function() {
    ...
    $('.input-number').change(function() {
        ...
        if(valueCurrent == maxValue) {
            ...
            valueStock = 'En existencia';
        }

        $.ajax({
            type: 'POST',
            url: url,
            data: {newQuantityValue:valueCurrent, newStockValue:valueStock},
            ...
            success: function(data) {
                ...
                $('#stock_' + data.product.id).hide();
                $('#stock_' + data.product.id).text(valueStock).delay(500).fadeIn();

                $('#money').hide();
                $('#money').text('$' + newTotalMoney).delay(500).fadeIn();

                $('#money2').hide();
                $('#money2').text('$' + newTotalMoney).delay(500).fadeIn();
